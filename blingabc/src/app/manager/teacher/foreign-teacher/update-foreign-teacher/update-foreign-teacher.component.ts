...
import {
  ...
  ITdDataTableSelectEvent, IPageChangeEvent, TdDialogService, TdLoadingService
} from '@covalent/core';
...
@Component({
  selector: 'app-update-foreign-teacher',
  templateUrl: './update-foreign-teacher.component.html',
  styleUrls: ['./update-foreign-teacher.component.scss']
})
export class UpdateForeignTeacherComponent implements OnInit {
  form: FormGroup;
  id: number;
  foreignId: number;
  ...
  lessonColumns: ITdDataTableColumn[] = [
    ...
    {name: 'registeredStudentCount', label: 'Registered students'},
      ...
      if (Boolean(parms['id'])) {
        Observable.zip(
          ...
          if (result[0].code === '10000' && result[0].msg === 'ok') {
            ...
            this.lessonTotal = result[0].data.total;
          }
          ...
          if (result[1].code === '10000' && result[1].msg === 'ok') {
            ...
            this.classTotal = result[1].data.total;
          }
        });
      }
    });
