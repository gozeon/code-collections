...
void producer_task(std::stop_token stoken, MagicRingBuffer &mrb, std::shared_ptr<spdlog::logger> log)
{
	log->info("producer_task 开始工作");
	...
	cv.notify_all();
	log->info("producer_task 结束工作");
}

void consumer_task(std::stop_token stoken, MagicRingBuffer &mrb, std::shared_ptr<spdlog::logger> log)
{
	log->info("consumer_task 开始工作");
	...
	while (true)
	{
		...
		if (stoken.stop_requested())
		{
			log->warn("收到停止信号, 还有 {} 数据未处理!", mrb.size());
			break;
		}
		...
		log->info("{}", ptr[0]);
		mrb.pop(1024);
		log->info("还剩下 {} 字节", mrb.size());
	}

	log->info("consumer_task 结束工作");
}

int main()
{
	auto console_sink = std::make_shared<spdlog::sinks::stdout_color_sink_mt>();
	auto prod_log = std::make_shared<spdlog::logger>("PRODUCER", console_sink);
	auto cons_log = std::make_shared<spdlog::logger>("CONSUMER", console_sink);

	std::signal(SIGINT, signal_handler);

	MagicRingBuffer mrb{4 * 1024};

	std::jthread t1(consumer_task, global_stop_source.get_token(), std::ref(mrb), cons_log);
	std::jthread t2(producer_task, global_stop_source.get_token(), std::ref(mrb), prod_log);
