...
- **尺寸沿用参考图**：尺寸选择器默认「跟随参考图」，有参考图时生成尺寸直接沿用参考图宽高，修改指令不会把画面改成默认的方框（超出模型允许范围时等比缩放，非 8 的倍数对齐到 8）；换成具体宽高比（1:1 / 4:3 / 3:4 / 16:9 / 9:16）后按该宽高比 + 分辨率档位推算，此时选择器右侧的「参考图尺寸」标签会显示实际生成尺寸。参考图上传时最多压缩到 2048px 以控制请求体，但按原图尺寸生成：前端把原始宽高随请求发送（`referenceSize`），服务端也会自行解析参考图（`image-dimensions.ts`）兜底；没有参考图时回退到默认宽高比与分辨率。
...
└─ lib/
   ...
   ├─ image-dimensions.ts # 读参考图宽高（解析 PNG / JPEG / GIF / WebP 文件头，供「跟随参考图尺寸」）
