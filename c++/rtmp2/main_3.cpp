...
void record_rtmp(const std::string &rtmp_url)
{
	...
	while (keep_running)
	{
		...
		if (!RTMP_SetupURL(rtmp, (char *)rtmp_url.c_str()) || !RTMP_Connect(rtmp, NULL) || !RTMP_ConnectStream(rtmp, 0))
		{
			...
			for (int i = 0; i < sleep_sec && keep_running; ++i)
			{
		...
		while (keep_running)
		{
			...
			{
				std::println(stderr, "[错误] RTMP_Read {} 秒无数据，准备重连", read_timeout_threshold.count());
				break;
			}
			int readSize = RTMP_Read(rtmp, reinterpret_cast<char *>(buffer), sizeof(buffer));
			if (readSize <= 0)
			{
				std::println(stderr, "[错误] RTMP_Read 返回异常，准备重连");
				break;
			}
		...
		std::println("[重连] 正在清理当前连接资源并尝试重连...");
		RTMP_Close(rtmp);
		RTMP_Free(rtmp);
