...
@Component({
  ...
  templateUrl: './process-teacher-launch.component.html',
  styleUrls: ['./process-teacher-launch.component.scss']
})
export class ProcessTeacherLaunchComponent implements OnInit {
  form: FormGroup;
  id: number;
  foreignId: number;
  ...
  onSubmit(): void {
    ...
    this._foreignTeacherService.updateProcessTeachersLaunch(data).subscribe(r => {
  ...
  openChangeForeignTeacher(type: string, row: any): void {
    this._dialog.open(ChangeForeignTeacherComponent, {width: '50%', data: {id: this.id, name: this.form.value.name}})
      .afterClosed()
      .subscribe(data => {
        if (data) {
          switch (type) {
