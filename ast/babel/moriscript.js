	...
	return {
		visitor: {
			...
			ArrayExpression(path) {
				path.replaceWith(
					t.callExpression(moriMethod("vector"), path.node.elements)
				);
			},
			AssignmentExpression(path) {
				...
				if (t.isMemberExpression(lhs)) {
					...
					path.replaceWith(
						t.callExpression(moriMethod("assoc"), [
							lhs.object,
							lhs.property,
							rhs
						])
			...
			MemberExpression(path) {
				...
				path.replaceWith(
					t.callExpression(moriMethod("get"), [
						path.node.object,
						path.node.property
					])
				);
				...
				if (t.isMemberExpression(callee)) {
					if (
						...
						path.node.arguments = path.node.arguments.map(function(expr) {
							return t.callExpression(moriMethod("toJs"), [expr]);
						});
					}
				}
			}
